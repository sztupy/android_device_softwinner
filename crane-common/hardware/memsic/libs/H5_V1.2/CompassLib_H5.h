...
#define COMPASSLIB_TYPES
	...
	#elif defined __arm	//type definition for ADS1.2
...
#define COMPASSLIB_H5_VERSION_HI 1
#define COMPASSLIB_H5_VERSION_LO 0
...
typedef struct
{
	...
	int16 Pitch;
	int16 Yaw;
} COMPASSLIB_H5_ORIENTATION;

// debugging information
typedef struct
{
	uint8 Data[12];
} COMPASSLIB_H5_DEBUG_INFO;
...
typedef enum {
	COMPASSLIB_H5_INIT_OK = 0x00,
	COMPASSLIB_H5_INIT_ERROR_BAD_STRUCT = 0x01,
	COMPASSLIB_H5_INIT_ERROR_WRONG_VERSION = 0x02,
	COMPASSLIB_H5_INIT_ERROR_NVM_CORRUPT = 0x04
} COMPASSLIB_H5_INIT_RESULT;

// enumerate library states
typedef enum
{
	COMPASSLIB_H5_STATE_UNAVAILABLE, // compass was not properly initialized and is unavailable
	COMPASSLIB_H5_STATE_OFF,         // compass is inactive - no features available
	COMPASSLIB_H5_STATE_ON,          // compass is active
	COMPASSLIB_H5_STATE_BUSY         // compass is active and is currently auto-calibrating - needs additional CPU cycles
} COMPASSLIB_H5_STATE;
...
typedef uint16 (*COMPASSLIB_H5_NVM_READ_FUNCTION)(uint16 offset);

// define a pointer to a function that performs writing to non-volatile memory
typedef uint8 (*COMPASSLIB_H5_NVM_WRITE_FUNCTION)(uint16 offset, uint8 data);

// define the structure that must be passed to CompassLib_H5_Init()
typedef struct
{
	// size of this structure
	// helps library verify that proper version of structure is used in the application
	// user should pass the value sizeof(COMPASSLIB_H5_INIT_STRUCT)
	uint16 Size;

	// version number of the compass library
	// user should pass the values COMPASSLIB_H5_VERSION_HI and COMPASSLIB_H5_VERSION_LO
	uint8 VersionHi;
	uint8 VersionLo;

	// pointer to NVM read routine
	COMPASSLIB_H5_NVM_READ_FUNCTION NVM_Read;

	// pointer to NVM write routine
	COMPASSLIB_H5_NVM_WRITE_FUNCTION NVM_Write;
} COMPASSLIB_H5_INIT_STRUCT;

// call this function to initialize the library
COMPASSLIB_H5_INIT_RESULT CompassLib_H5_Init(const COMPASSLIB_H5_INIT_STRUCT * api);
...
COMPASSLIB_H5_STATE CompassLib_H5_Task(uint16 milliseconds);

// pass data into the compass library
void CompassLib_H5_DataIn(int16 gx, int16 gy, uint16 hx, uint16 hy, uint16 hz);
...
void CompassLib_H5_ClearCalibration(void);

// used to enable/disable auto calibration features
// features are enabled at boot by default
void CompassLib_H5_EnableAutoCalibration(uint8 enable);
...
COMPASSLIB_H5_BUILD_DATE CompassLib_H5_GetBuildDate(void);

// get the calibration quality factor (0 - 100%)
uint8 CompassLib_H5_GetCalibrationQualityFactor(void);

// returns number of bytes of non-volatile memory used by the library
uint16 CompassLib_H5_GetNVMBlockSize(void);

// indicates magnetic noise level (0 - 100%)
// 0 = no noise
// 100 = severe noise
uint8 CompassLib_H5_GetMagneticNoiseLevel(void);

// returns the current operating state of the library
COMPASSLIB_H5_STATE CompassLib_H5_GetOperatingState(void);

// library version information
// high-byte of result is major version
// low-byte of result is minor version
uint16 CompassLib_H5_GetVersion(void);

// returns whether auto calibration features are presently enabled
uint8 CompassLib_H5_IsAutoCalibrationEnabled(void);

// used for debugging
COMPASSLIB_H5_DEBUG_INFO CompassLib_H5_GetDebugInfo(void);
...
// function analyzes gravity vector (gx, gy) and current magnetic enviroment (hx,hy,hz)
// and returns orientation
// magnetic information is passed in as raw LSBs, and is adjusted by to the current auto-calibration
COMPASSLIB_H5_ORIENTATION CompassLib_H5_CalculateOrientation(int16 gx, int16 gy, uint16 hx, uint16 hy, uint16 hz);
Geomagnetic_Offset_Value CompassLib_H5_Get_Offset(void);
#ifdef __cplusplus
}
#endif
