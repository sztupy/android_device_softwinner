...
AudioHardware::AudioHardware() :
    ...
    mPaDevHandle(NULL)
{
	mPaDevHandle = open( "/dev/pa_dev", O_RDONLY );
	if (mPaDevHandle <= 0)
	{
		LOGE("open pa device failed");
		return;
	}
	LOGV("open PA ok");
...
// add interfaces here
status_t AudioHardware::setAudioOutMode(unsigned int mode)
{
	LOGD("setAudioOutMode %d", mode);
	return alsa_setAudioOutMode(mode);
}

int AudioHardware::getAudioOutMode()
{
	return alsa_getAudioOutMode();
}

status_t AudioHardware::setAudioPaOnOff(bool status)
{
	int ret = -1;
	int paCMD = status ? PA_OPEN : PA_CLOSE;
	LOGD("to set PA %s", status ? "on" : "off");
	
	if (mPaDevHandle == NULL)
	{
		LOGD("setAudioPaOnOff failed, PA device not opened");
		return -1;
	}
	
	ret = ioctl(mPaDevHandle, paCMD, 0);
	if (ret < 0)
	{
		LOGE("setAudioPaOnOff failed");
		return ret;
	}
...
ssize_t AudioHardware::AudioStreamOutALSA::write(const void* buffer, size_t bytes)
{
    ...
    if((tmp_cnt & 63)==0 && bytes > 4){
    	char *tmp_buffer = (char *)buffer;
    	LOGV("alsa write: %02x %02x %02x %02x",tmp_buffer[0],tmp_buffer[1],tmp_buffer[2],tmp_buffer[3]);
    }
    tmp_cnt++;
#endif
