...
AUDIO_OUT_MODE g_cur_mode = AUDIO_OUT_DEFAULT;

int alsa_getAudioOutMode()
{
	return g_cur_mode;
}

int alsa_setAudioOutMode(unsigned int mode)
{
	int ret = 0;
	switch (mode)
	{
		case AUDIO_OUT_DEFAULT:
		case AUDIO_OUT_CODEC:
			LOGD("SwitchAudioOutMode AUDIO_OUT_CODEC");
			g_cur_mode = AUDIO_OUT_CODEC;
			property_set(PROP_AUDIO_OUT_MODE_KEY, PROP_AUDIO_OUT_CODEC);
			break;
		case AUDIO_OUT_HDMI:
			LOGD("SwitchAudioOutMode AUDIO_OUT_HDMI");
			g_cur_mode = AUDIO_OUT_HDMI;
			property_set(PROP_AUDIO_OUT_MODE_KEY, PROP_AUDIO_OUT_HDMI);
			break;
		default:
			LOGE("SwitchAudioOutMode error mode: %d not support", mode);
			ret = -1;
			break;
	}
...
struct pcm *pcm_open(unsigned flags)
{
    ...
    property_get(PROP_AUDIO_OUT_MODE_KEY, prop_value, PROP_AUDIO_OUT_DEFAULT);
    LOGV("prop_value = %s", prop_value);
    if(!strcmp(prop_value, PROP_AUDIO_OUT_CODEC))
        g_cur_mode = AUDIO_OUT_CODEC;
    else if(!strcmp(prop_value, PROP_AUDIO_OUT_HDMI))
        g_cur_mode = AUDIO_OUT_HDMI;
	else
	{
		LOGW("property_get error mode, use default mode");
		g_cur_mode = AUDIO_OUT_CODEC;
	}
    ...
    if (flags & PCM_IN) {
    	...
    	if (g_cur_mode == AUDIO_OUT_CODEC){
        	LOGD("AUDIO_OUT_CODEC");
			dname = "/dev/snd/pcmC0D0p";
    	}
		else if (g_cur_mode == AUDIO_OUT_HDMI){
        	LOGD("AUDIO_OUT_HDMI");
        	dname = "/dev/snd/pcmC1D0p";
		}
		else {
			LOGW("ERROR MODE, use default mode");
			dname = "/dev/snd/pcmC0D0p";
		}
    }
