...
acquire_wake_lock(int lock, const char* id)
{
    ...
    //LOGV("acquire_wake_lock lock=%d id='%s'\n", lock, id);
...
{
    //LOGV("set_last_user_activity_timeout delay=%d\n", ((int)(delay)));
...
set_screen_state(int on)
{
    ...
    //LOGV("go_to_sleep eventTime=%lld now=%lld g_error=%s\n", eventTime, systemTime(), strerror(g_error));
